...
Features of baseline data: After filtering, `r nrow(mci_baseline_df)` subjects met the inclusion criterion of no MCI at baseline, of which `r mci_baseline_df %>% filter(age_at_onset > current_age) %>% summarise(n())` developed MCI. The average baseline age was `r mci_baseline_df %>% summarise(mean(current_age))` years. The proportion of women in the study who are APOE4 carriers is `r prop_women_apoe4` (or `r prop_women_apoe4*100`%).