...
The longitudinal csv is in a wide format, so it was tidyed. However, in
order to combine the longitudinal with the baseline df, the wide format
was needed and the study_id/id column had to have the same name. Once
they were compatible, they were merged over the common subjects using
inner_join. The total number of subjects in the baseline df was 479, the
total number in the longitudinal df were 487, and the total number
(overlapping) in the merged df was 471. This means that some subjects
appeared in only one or the other df, but the majority overlapped.
